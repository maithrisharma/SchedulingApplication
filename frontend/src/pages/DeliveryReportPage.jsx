// src/pages/DeliveryReportPage.jsx — FINAL, FULL, NO ERRORS, 100% LIKE UNPLACED PAGE
...
export default function DeliveryReportPage() {
  ...
  const columns = [
    { field: "OrderNo", headerName: "OrderNo", minWidth: 160, flex: 1, renderHeader: () => <HeaderWithFilter label="OrderNo" active={valueFilters.OrderNo?.active} onClick={(e) => { setActiveValueField("OrderNo"); setValueFilterAnchor(e.currentTarget); setValueFilterSearch(""); }} /> },
    { field: "SupposedDeliveryDate", headerName: "Supposed Delivery", minWidth: 220, flex: 1, renderCell: (p) => fmtDate(p.row.SupposedDeliveryDate), renderHeader: () => <HeaderWithFilter label="SupposedDeliveryDate" active={dateFilters.SupposedDeliveryDate?.selected.length > 0 || dateFilters.SupposedDeliveryDate?.includeNull} onClick={(e) => { setActiveDateField("SupposedDeliveryDate"); setDateFilterAnchor(e.currentTarget); }} /> },
    { field: "DeliveryAfterScheduling", headerName: "Delivered After Scheduling", minWidth: 240, flex: 1, renderCell: (p) => fmtDate(p.row.DeliveryAfterScheduling), renderHeader: () => <HeaderWithFilter label="DeliveryAfterScheduling" active={dateFilters.DeliveryAfterScheduling?.selected.length > 0 || dateFilters.DeliveryAfterScheduling?.includeNull} onClick={(e) => { setActiveDateField("DeliveryAfterScheduling"); setDateFilterAnchor(e.currentTarget); }} /> },
    { field: "DaysLate", headerName: "DaysLate", minWidth: 140, flex: 1, renderHeader: () => <HeaderWithFilter label="DaysLate" active={valueFilters.DaysLate?.active} onClick={(e) => { setActiveValueField("DaysLate"); setValueFilterAnchor(e.currentTarget); setValueFilterSearch(""); }} /> },
  ];
  ...
  return (
    <ThemeProvider theme={gridTheme}>
      <Box sx={{ p: 4, bgcolor: "#f1f5f9", minHeight: "100vh" }}>
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
              ...
              <Stack direction="row" spacing={2}>
                ...
                <Button variant="contained" startIcon={<Download />} disabled={!scenario} onClick={() => apiFetchFile(`/visual/${scenario}/delivery-excel`, `${scenario}_delivery.xlsx`)}>
                  Download Excel
                </Button>
              </Stack>
            </Stack>
        ...
        {/* DATE FILTER POPOVER — COLLAPSIBLE WITH ICONS */}
